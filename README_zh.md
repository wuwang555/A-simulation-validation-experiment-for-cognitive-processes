...
| 元素 | 颜色/样式 | 含义 |
|------|-----------|------|
| **节点** | 浅蓝色圆点，黑色边框 | 代表概念节点（如“算法”、“神经网络”、“能量守恒”等） |
| **节点标签** | 黑色文字 | 概念的名称 |
| **普通边** | 蓝绿 → 红紫渐变，粗细可变 | 表示概念之间的认知关联。**颜色**：越偏蓝绿表示当前认知能耗越低（学习充分），越偏红紫表示能耗越高（尚不熟悉）。**粗细**：越粗表示能耗越高（阻力大），越细表示能耗越低（流畅） |
| **硬遍历路径** | 蓝色实线箭头 | 系统执行“硬遍历”——沿着已有低能耗路径进行高效、确定的认知操作，巩固已有知识 |
| **软遍历尝试** | 橙色虚线箭头 | 软遍历过程中尝试过的多个可能方向，体现探索性 |
| **软遍历实际路径** | 红色实线箭头 | 软遍历最终选择的路径，用于探索新关联或跨域连接 |
| **高亮节点** | 黄色圆点，黑色边框 | 当前遍历路径上经过的节点，表示当前正在被激活/访问的概念 |
| **标题栏** | 左上角文字 | 显示当前迭代次数和网络全局平均能耗 |

- 边的颜色和粗细会随迭代动态变化，反映认知能耗随学习/遗忘而演化。
- 硬遍历通常只沿一条路径前进（蓝色实线），软遍历会先尝试多个方向（橙色虚线），再确定实际路径（红色实线）。
- 节点位置也会随网络结构变化而缓慢移动（如果启用了动态布局），体现认知空间的重构。
