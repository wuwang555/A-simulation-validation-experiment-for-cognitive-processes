...
class ProbabilisticGraphEnhancement:
    ...
    def find_high_mi_clusters(self, threshold=1.0):
        ...
        for concept1 in self.co_occurrence:
            if concept1 in processed:
                continue

            cluster = [concept1]
            for concept2 in self.co_occurrence:
...
class AdjustedSubjectiveCognitiveGraph:
    ...
    def traverse_path(self, path, traversal_type="hard"):
        ...
        for i in range(len(path) - 1):
            ...
            if self.G.has_edge(u, v):
                ...
                if traversal_type == "hard":
                    learning_rate = base_rate * (0.7 + 0.3 * similarity) * individual_learning_variation * prob_factor
                else:
                    learning_rate = base_rate * 0.9 * individual_learning_variation * prob_factor
    ...
    def _probabilistic_compression(self):
        ...
        # 使用概率信息寻找压缩候选
        high_mi_clusters = self.semantic_network.probabilistic_enhancement.find_high_mi_clusters(threshold=0.5)

        for cluster in high_mi_clusters:
            if len(cluster) >= 3 and len(cluster) <= 6:
                # 选择中心节点（与其他节点互信息最高的）
                center_candidate = max(cluster,
                                       key=lambda node: sum(
                                           self.semantic_network.probabilistic_enhancement.mutual_info[node].get(other,
                                                                                                                 0)
                                           for other in cluster if other != node
                                       ))
                ...
                if success:
                    print(f"概率压缩: {center_candidate} <- {related_nodes}")
                    break
...
def run_individual_simulation(individual_id, individual_params, max_iterations=5000):
    """运行单个个体模拟"""
    print(f"\n=== 开始模拟 {individual_id} ===")
