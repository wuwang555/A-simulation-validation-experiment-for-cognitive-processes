...
% 群作用示意（调整位置：向右下移动）
\node[space] (space) at (7, 1.5) {};  % 调整：原(6,-1)，现(7,1.5)
\node[circle, draw, fill=white] (state1) at (6.5, 2) {$G_1$};
\node[circle, draw, fill=white] (state2) at (7.5, 2) {$G_2$};
\node[circle, draw, fill=white] (state3) at (7, 1) {$G_3$};
...
% 标注（调整位置）
\node[text width=4cm, align=center, font=\small] at (3, -3) {  % 调整：原(2,-4)，现(3,-3)
