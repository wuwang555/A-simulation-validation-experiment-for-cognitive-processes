...
\begin{abstract}
回应Bowers等人（2023）对认知模型必须经过严格测试的方法论批评，本文构建一个\textbf{基于第一性原理的思想实验平台}，从几何、代数和动力学三个维度为认知过程提供统一的形式化模型。系统基于两个核心公设：(1) 认知状态可表征为时空中的动态网络（认知图）；(2) 网络演化受能量动力学支配，其核心优化目标是认知自由能（广义认知能耗）最小化。在此公理基础上，引入群论代数结构描述认知操作的封闭性与可逆性，并推导出“概念压缩”与“第一性原理迁移”是系统为达成能耗优化而自然涌现的宏观行为模式。

为观察公理系统的内生解释力，我们在包含51、71、91和111个跨领域概念的四个规模上进行计算实验，对比代表外部干预的“预设算法模型”与仅由两个核心公设驱动的“自然涌现模型”。结果显示，自然涌现模型在四个规模下分别实现86.9\%至47.9\%的能耗降低，且能耗下降曲线可被幂律函数完美拟合（拟合优度\(R^2>0.99\)），揭示了系统时间演化的精确标度律。进一步，概念节点作为压缩中心的出现次数及节点在压缩事件中的总参与度均服从Zipf幂律分布（斜率约-1.35，\(R^2>0.6\)），表明系统的空间结构自发形成了与自然语言、城市规模等复杂系统一致的长尾统计规律。这些发现为能量最小化作为认知组织基本原则提供了多维度的客观证据。

核心发现：系统在不同概念规模下均展现出与人类认知高度一致的组织原则——概念压缩形成稳定的知识结构，原理迁移实现跨领域创造性连接，且能根据认知负荷自适应调整探索-利用策略。本工作旨在为理解智能本质提供新的数学视角，而非构建工程系统。该框架统一解释分散的认知现象，并对当前基于统计关联的人工智能范式构成根本性质疑，为从数据驱动转向原理驱动的认知建模提供了基于第一性原理的计算验证范例。
...
受物理学中“最小作用量原理”和数学中群论的启发，本文提出一个核心论点：认知过程的底层驱动力，是对“认知能耗”这一内部代价的优化，且认知操作具有代数封闭性和结构保持性。基于此，我们构建了一个以“认知时空”与“能量动力学”为第一性原理的公理化系统。为给这个动态系统提供更严格的数学基础，我们进一步引入了群论结构，\textbf{观测到}认知操作在复合下构成半群，并发现认知对称性与守恒量之间存在对应关系。这些代数性质并非人为设定，而是从公理系统中自然延伸出的形式化描述。需要强调的是，这里的代数结构仅作为一种描述语言，而非系统演化的驱动机制。认知操作的实际演化完全由第3节定义的能量动力学决定。
...
\textbf{研究范式定位：} 需要特别说明的是，本研究并非旨在构建一个能够精确拟合人类行为数据的认知模型，也不是为了与特定人工智能系统进行性能对标。相反，我们的目标是建立一个\textbf{基于第一性原理的思想实验平台}。Woensdregt等人 \cite{woensdregt2024lessons} 曾指出，在实证证据稀疏或间接的领域，理论发展需要遵循“良性实践”：明确假设、明确备选理论、计算/形式化建模、寻求外部一致性、三角验证。本文的工作可以看作是在认知计算建模领域对这些实践的一次具体应用：我们明确陈述两个公设（实践i），梳理并对比现有理论（实践ii），构建可计算模型进行思想实验（实践iii），寻求与多个认知理论的外部一致性（实践iv），并通过多规模、多类型实验进行三角验证（实践v）。这一方法论立场贯穿全文。特别地，我们在实验中不仅报告了能耗优化百分比，还进一步揭示了系统演化过程中隐藏的数学规律：能耗下降曲线的幂律标度行为（拟合优度\(R^2>0.99\)）以及概念节点参与度的Zipf分布（幂律指数约\(-1.35\)，\(R^2>0.6\)）。这些客观的统计规律为能量最小化驱动认知自组织提供了超越单纯性能对比的坚实证据。
...
% 定义节点位置（调整间距，避免重叠）
\node[concept] (v1) at (-3,1) {算法};  % 左移
\node[concept] (v2) at (0,2) {递归};   % 上移
\node[concept] (v3) at (3,1) {神经网络};  % 右移
\node[concept] (v4) at (-1,-2) {模式识别};  % 左下
\node[concept] (v5) at (4,-2) {优化};   % 右下
\node[energy] (v6) at (8,-3) {认知能耗};  % 右移更多

% 连接边并标注权重（使用弯曲边避免交叉）
\draw[->] (v1) -- node[midway, above] {0.85} (v2);
\draw[->] (v1) -- node[midway, above] {0.78} (v3);
\draw[->] (v2) -- node[midway, right] {0.92} (v3);
\draw[->] (v2) to[out=270, in=90] node[midway, left] {0.67} (v4);  % 弯曲边
\draw[->] (v3) -- node[midway, right] {0.81} (v5);
\draw[->] (v4) to[out=0, in=180] node[midway, below] {0.74} (v5);  % 弯曲边
\draw[->] (v5) -- node[midway, right] {0.63} (v6);
\draw[->] (v1) to[out=270, in=180] node[midway, left] {0.45} (v4);  % 弯曲边
\draw[->] (v3) to[out=270, in=0] node[midway, right] {0.58} (v5);   % 弯曲边

% 标注（调整位置，避免遮挡）
\node[text width=4cm, align=center, font=\small] at (5,4) {认知图 $G(V,E,t)$ \\ 时间 $t$: 迭代次数};  % 右移
\node[text width=3cm, align=center, font=\small] at (-2,4) {节点: 概念 \\ 边权重: 认知能耗};  % 左移

% 能量函数示意（调整位置到右上，避免与节点冲突）
...
\begin{small}
    说明：公式(6)是公设二的一个具体实例，而非对现实认知的拟合。该形式因直观、可分解且在文献中有先例而被选用；任何满足公设二的能耗函数均可驱动系统，本工作仅取最简单的一种进行演示。
\end{small}
...
压缩势是一个纯粹的观测指标，它不参与系统的演化决策，仅用于事后分析被系统实际压缩的集群的统计特征。它与第 3.3 节中定义的压缩触发条件（公式 \ref{eq:compression_condition}）本质上是等价的——压缩势反映了集群内部与外部的能耗对比，而触发条件则是该对比超过某一阈值时的具体表现。引入压缩势使我们能够以连续、客观的方式考察压缩行为的质量，并为第 7.3 节的规模效应分析提供量化依据。
...
引入代数结构有助于形式化描述认知过程：封闭性提示认知操作的自包含性，结合律对应思维过程的连贯性，单位元对应“维持注意力”等不变操作，逆元的存在性则反映了认知过程的（不）可逆性。这些代数性质为认知操作提供了一种可计算的数学语言，但不参与系统的决策与演化——演化仍由能量动力学独立驱动。
...
这一敏感性分析表明，在 0.5 至 0.75 的宽阈值范围内，压缩现象稳定存在。对于思想实验而言，参数选择的“最优性”并非核心问题，关键在于现象的稳健性——只要给定合理参数范围，系统都能自发生成具有统计规律的压缩与迁移行为，即足以证明公设的有效性。本分析旨在回应参数选择主观性的关切，并为后续研究在不同知识领域应用本框架时选择合适阈值提供参考依据。
...
% ========== SUBSECTION 7.9: 计算效率分析：低能耗、低成本的自然涌现 ==========
\subsection{计算效率分析：低能耗、低成本的自然涌现}
为回应关于“认知图论是否只是一个计算昂贵的玩具”的潜在疑问，我们进一步测算了系统的实际运行时间。所有实验均在普通笔记本电脑（Intel i7, 16GB RAM, 无GPU加速）上完成，使用纯 Python 实现。表 \ref{tab:time_comparison} 对比了仅执行核心演化（无任何观测）与完整演化（含压缩/迁移检测）在四个规模下的平均耗时。

\begin{table}[H]
\centering
\caption{核心演化与完整演化的平均运行时间（秒，10,000次迭代，3个体平均）}
\label{tab:time_comparison}
\begin{tabular}{lccc}
\toprule
\textbf{规模} & \textbf{核心演化（无检测）} & \textbf{完整演化（含检测）} & \textbf{时间比} \\
\midrule
51 概念 & 2.19 & 5.80 & 2.65x \\
71 概念 & 3.25 & 8.58 & 2.64x \\
91 概念 & 4.51 & 11.86 & 2.63x \\
111 概念 & 7.19 & 18.28 & 2.54x \\
\bottomrule
\end{tabular}
\end{table}

即使是最复杂的 111 概念网络，完整演化也仅需约 18 秒即可完成 10,000 次迭代（涵盖数千次压缩/迁移检测）。核心演化（即纯能量动力学）只需 7 秒左右。这一计算开销远低于任何基于深度学习的模型——后者训练动辄消耗数千 GPU 小时，单次前向推理也可能消耗毫瓦级功率。我们的模型在普通笔记本电脑上即可运行，其能耗甚至可以忽略不计。

这种极致的计算效率源自两个根本原因：一是系统演化仅依赖局部能量更新，无反向传播或全局优化；二是我们故意回避了任何“大”的数据集——系统无需预训练，所有知识来自初始语义网络的定义。这从侧面印证了“原理驱动”模型在能源效率上的天然优势，与当前 AI 领域对大算力的依赖形成鲜明对比。

这一结果也呼应了第 8.2 节的论点：从原理出发的认知模型，有可能绕过暴力计算，以极低的能量消耗实现高阶认知功能的涌现。

% ========== SUBSECTION 7.10: 概念出现频率的幂律分布：认知系统的无标度特征 ==========
...
\textbf{实证验证的广度与深度不足}：当前验证主要基于计算指标，未与人类行为数据或脑成像数据直接对比。但需说明的是，本研究的定位并非实证拟合，而是提供一个可计算的“可能世界”。未来若需检验其与真实认知的对应关系，可设计心理学实验检验模型预测，或将模型输出与脑活动数据进行多变量模式分析——但此已超出本文作为思想实验平台的预设范围。认知科学的应用前景广阔，如Carlson等 \cite{carlson2025attending} 对气候变化关注的研究所示，认知理论可指导现实问题的解决。
