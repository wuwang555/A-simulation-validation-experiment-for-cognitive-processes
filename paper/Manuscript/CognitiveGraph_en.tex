...
\documentclass[10pt,onecolumn]{article}  % 10pt, single column
...
\usepackage{makecell}
\usepackage{multirow}
\usepackage[top=2.2cm, bottom=2.2cm, left=2.2cm, right=2.2cm]{geometry}  % Slightly reduced margins for more content
\usepackage{setspace}
\setstretch{1.0}  % Single line spacing

% Set English captions for tables, figures, references, etc.
\ctexset{
    figurename = Figure,
    tablename = Table,
...
% Image format suggestions (comment for authors)
% - Use TIFF, EPS, JPEG; TIFF with LZW compression
% - Color images: ≥300dpi; B&W: ≥500dpi; line art: ≥1000dpi
% - Color mode: RGB
% - Line thickness: 0.25pt to 1.5pt
% - File size: ≤20MB
...
\section{Algebraic Structures in Cognitive Graph Theory: A \\Group-Theoretic Description}
...
\begin{lstlisting}[language=Python, caption={Cognitive State Enumeration and Manager (\texttt{cognitive\_states.py})}, label=lst:state_manager]
...
\begin{lstlisting}[language=Python, caption={Operation Selection and Traversal in the (\texttt{cognitive\_graph.py})}, label=lst:operations]
...
\begin{lstlisting}[language=Python, caption={Semantic Enhancement and Energy Optimization Class (\texttt{enhanced\_model.py})}, label=lst:enhanced]
