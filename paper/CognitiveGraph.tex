...
\textbf{与自由能原理的对比}：本工作与Karl Friston的自由能原理共享“能量最小化”这一核心原则，但研究层次与实现机制不同。自由能原理旨在解释神经层面的预测处理与稳态维持，是一个深刻的哲学框架；认知图论则在认知的抽象层面构建了具体的、可操作的动力学系统。两者可视为不同层次上的平行探索：自由能原理解释“为什么”（Why），认知图论尝试描述“如何”（How）。
...
我们的理论框架与神经科学的多个前沿发现形成了有趣的对应关系。能量效率原则与人脑仅占体重2\%却消耗20\%基础代谢率的能量预算相呼应；概念压缩密度随规模衰减的现象与发育过程中突触修剪的过程存在类比；模块化组织的结构对应大脑功能模块的自然边界；神经可塑性的动力学模拟则与长时程增强、长时程抑制等机制形成映射。这些对应关系提示，能量最小化可能不仅是认知层面的组织原则，也可能是神经层面结构优化的普遍原理。
...
\textbf{个体差异建模的精细度不足}：当前仅通过随机初始化引入个体差异，缺乏基于认知风格的系统性建模。未来可基于心理学量表建立认知风格参数化模型，模拟从儿童到专家的认知发展连续体。

\textbf{多模态感知运动的整合缺失}：当前模型完全基于抽象概念，缺乏感知和运动的具身基础。未来可结合多模态神经网络，在机器人平台上实现具身验证。

\textbf{实证验证的广度与深度不足}：当前验证主要基于计算指标，缺乏与人类行为数据和脑成像数据的直接对比。未来需设计心理学实验检验模型预测，并将模型输出与脑活动数据进行多变量模式分析。
...
本附录提供第2.1节中作为对照基线的预设算法模型的详细实现。

% ========== B.1 核心类定义 ==========
\subsection{核心类定义}

\begin{lstlisting}[language=Python, caption=预设算法模型主类]
class SemanticEnhancedCognitiveGraph(BaseCognitiveGraph):
    def __init__(self, concepts, meta_structure_map=None, seed=None):
        super().__init__(concepts, seed=seed)
        self.meta_structure_map = meta_structure_map or {}
        self.cognitive_state = "FOCUSED"
    ...
    def _init_semantic_weights(self):
        """基于语义相似度初始化边权重"""
        for i, vi in enumerate(self.concepts):
            for j, vj in enumerate(self.concepts):
                if i != j:
                    sim = self._semantic_similarity(vi, vj)
                    # 权重公式: 2.0 - 1.5 * sim
                    self.weights[i][j] = 2.0 - 1.5 * sim
    
    def _semantic_similarity(self, c1, c2):
        """计算语义相似度（结合元结构）"""
        # 基础相似度基于定义文本
        base_sim = self._text_similarity(c1.definition, c2.definition)
        # 元结构增强
        meta1 = self.meta_structure_map.get(c1.name, "")
        meta2 = self.meta_structure_map.get(c2.name, "")
        meta_boost = 0.2 if meta1 and meta2 and meta1 == meta2 else 0.0
        return min(1.0, base_sim + meta_boost)
\end{lstlisting}

% ========== B.2 认知状态管理 ==========
\subsection{认知状态管理}

预设算法模型包含显式的认知状态转移机制，基于马尔可夫过程。

\begin{lstlisting}[language=Python, caption=认知状态管理器]
class CognitiveStateManager:
    ...
    def update_cognitive_state(self):
        ...
        trans_probs = STATE_TRANSITION_MATRIX[current]
        rand = random.random()
        cum = 0
        for new_state, prob in trans_probs.items():
            cum += prob
            if rand <= cum:
                if new_state != current:
                    self.current_state = CognitiveState[new_state]
                    self._update_subjective_energy()
                break
    
    def _update_subjective_energy(self):
        if self.current_state == CognitiveState.FOCUSED:
            self.subjective_energy *= 0.95
        elif self.current_state == CognitiveState.EXPLORING:
            self.subjective_energy *= 1.05
        elif self.current_state == CognitiveState.INSIGHT:
            self.subjective_energy *= 0.9
        elif self.current_state == CognitiveState.FATIGUE:
            self.subjective_energy *= 1.1
        self.subjective_energy = max(0.5, min(3.0, self.subjective_energy))
\end{lstlisting}

% ========== B.3 操作选择与执行 ==========
\subsection{操作选择与执行}

\begin{lstlisting}[language=Python, caption=基于状态的操作选择]
def _select_operation_based_on_state(self):
    state = self.cognitive_state
    if state == "FOCUSED":
        probs = {"hard_traversal": 0.6, "compression": 0.3, "soft_traversal": 0.1}
    elif state == "EXPLORING":
        probs = {"soft_traversal": 0.5, "migration": 0.3, "hard_traversal": 0.2}
    elif state == "INSIGHT":
        probs = {"migration": 0.5, "compression": 0.4, "soft_traversal": 0.1}
    else:  # FATIGUE
        probs = {"forget": 0.7, "hard_traversal": 0.3}
...
% ========== C.5 涌现检测算法伪代码 ==========
\subsection{涌现检测算法伪代码}

\begin{lstlisting}[language=Python, caption=概念压缩检测算法]
def detect_compression(network, thresholds):
    candidates = []
    communities = detect_communities(network)
    for comm in communities:
        if len(comm) < thresholds['min_cluster_size']:
            continue
        center = find_center(comm, network)
        cohesion = compute_cluster_cohesion(center, comm, network)
        energy_sync = compute_energy_synchronization(center, comm, energy_history)
        emergence = (cohesion + energy_sync) / 2
        if emergence > thresholds['compression_synergy']:
            candidates.append({
                'center': center,
                'community': comm,
                'emergence_strength': emergence
            })
    return sorted(candidates, key=lambda x: -x['emergence_strength'])
\end{lstlisting}

\begin{lstlisting}[language=Python, caption=原理迁移检测算法]
def detect_migration(network, start, end, max_depth=5):
    pq = PriorityQueue()  # (path_energy, path)
    pq.put((0, [start]))
    best_paths = []
    while not pq.empty() and len(best_paths) < 10:
        energy, path = pq.get()
        current = path[-1]
        if current == end:
            best_paths.append((energy, path))
            continue
        if len(path) >= max_depth:
            continue
        for neighbor in network.neighbors(current):
            if neighbor not in path:
                new_energy = energy + network.weight(current, neighbor)
                pq.put((new_energy, path + [neighbor]))
    # 计算迁移效率
    direct_energy = network.weight(start, end) if network.has_edge(start, end) else float('inf')
    results = []
    for energy, path in best_paths:
        if energy < direct_energy - 0.1:
            efficiency = (direct_energy - energy) / direct_energy
            results.append((path, efficiency))
    return results
\end{lstlisting}
